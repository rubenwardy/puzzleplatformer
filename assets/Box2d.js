...
Crafty.c("Box2D", {
	...
	* @param fixed - Is the body fixed in rotation?
	...
	*/
	addFixture : function(setup) {
		...
		// Preconfigured fixture
			...
			fixDef.isSensor = (!isNaN(setup.isSensor)) ? setup.isSensor : false;
	...
	*/
	...
	contactWithFixture:function(comp,fixture){
		var finalresult = [];
		var fixtureA = fixture;
		
		var entitys = comp;
		if (!jQuery.isArray(comp))
			entitys = Crafty(comp);
		...
		init: function (gx, gy, ptm_ratio, doSleep) {
			...
			Crafty.bind("EnterFrame", function() {
				...
				for(var b = _world.GetBodyList(); b; b=b.GetNext()) {
					if (b.GetUserData() && (!b.GetUserData().tile_data || b.GetUserData().tile_data.physics.bodyType!="static")) {
						var sprite = b.GetUserData();
						var newpos = {
										x: b.GetPosition().x * _PTM_RATIO,
										y:b.GetPosition().y * _PTM_RATIO
									};
						if (newpos.x != sprite.x || newpos.y != sprite.y){
							var from = {x: sprite.x, y: sprite.y};
							sprite.attr(newpos);
							sprite.trigger("move",from);
						}else{
							sprite.attr(newpos);
						}
